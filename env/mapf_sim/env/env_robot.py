...
# from world import mobile_robot
from math import pi, cos, sin
import numpy as np
from collections import namedtuple
from collision_detection import collision_cir_cir, collision_cir_matrix, collision_cir_seg
import random
...
class env_robot:
    ...
    def random_start_goal(self):
        ...
        self.randomseed_epsiode +=1
        return start_list, goal_list

    def my_random_start_goal(self):
        ...
        g_list = [[1,1,0]]
        for i in range (num):
            goal = [[random.randint(2*i+1, 2*i+3)],[random.randint(2*i+1, 2*i+3)],[0]]
            start= [[random.randint(7-2*i, 9-2*i)],[random.randint(7-2*i, 9-2*i)],[0]]

            goal_list.append(np.array(goal))
            start_list.append(np.array(start))
        # print(start_list)
...
# 实时获得机器人的速度角速度信息
    ...
    def robots_reset(self, reset_mode=1, **kwargs):
        ...
        if reset_mode == 0:
            ...
            state_list, goal_list = self.random_start_goal()
            # state_list, goal_list = self.my_random_start_goal()
            # print("state_list", goal_list)
            # ss
            for i in range(self.robot_number):
                self.robot_list[i].init_state = state_list[i]
                self.robot_list[i].goal = goal_list[i]
        ...
        elif reset_mode == 5: #在规定范围里random 
           
            print("goal_list", type(goal_list))
            for i in range(self.robot_number):
                self.robot_list[i].init_state = state_list[i]
                self.robot_list[i].goal = goal_list[i]
                self.robot_list[i].reset(self.random_bear) 
    ...
    def total_states(self):
        ...
        # if len(obs_circular_list) == 0:
        #     obs_circular_list = 
        #print("robot_state_list:",robot_state_list, "nei_state_list:",nei_state_list, "obs_circular_list:",obs_circular_list, "obs_line_list:",obs_line_list)
