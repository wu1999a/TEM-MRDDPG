...
class env_plot:
    def __init__(self, width=10, height=10, components=dict(),  full=False, keep_path=False, map_matrix=None, offset_x = 0, offset_y=0, **kwargs):
        ...
        self.color_list = ['g', 'r', 'k', 'c', 'm', 'y', 'b', 'w']
    ...
    def init_plot(self, **kwargs):
        ...
        car_image_path = Path(current_file_frame).parent / 'robot.jpg'
    ...
    def draw_robots(self, robots, **kwargs):
        for robot,i in zip(robots.robot_list,self.color_list):
            self.draw_robot(robot,robot_color=i, goal_color=i,**kwargs)
    ...
    def draw_robot(self, robot, robot_color = 'k', goal_color='r', show_lidar=True, show_goal=True, show_text=True, show_traj=False, traj_type='-g', **kwargs):
        
        # print(robot_color,goal_color)
        x = robot.state[0][0]
        y = robot.state[1][0]
    ...
    def draw_car(self, car, goal_color='c', goal_l=2, text=False, show_lidar=True, show_traj=False, traj_type='-g', **kwargs):
        ...
        #         lx1 = wx - line_length * cos(line_rad_b) / 2s
        #         ly1 = wy - line_length * sin(line_rad_b) / 2

        #         self.car_line_list.append(self.ax.plot([lx0, lx1], [ly0, ly1], 'k-'))
        goal_arrow = mpl.patches.Circle(xy=(gx, gy), radius = 1, color=goal_color, alpha=0.5)
        ...
        # car image show
        # print(x,y)
        car_img = self.ax.imshow(self.init_car_img, extent=[x, x+car.length, y, y+car.width+3])
